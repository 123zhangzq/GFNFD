...
from utils import init_wandb, aspect_ratio_normalize,log_metrics, sample_uniform_per_bin, non_uniform_thermometer_encode
...
def train():
    ...
    savepath = os.path.join(output_dir, str(config["num_orders"] - config["num_riders"]), run_name)
    ...
    for epoch in range(start_epoch, config["epochs"]):
        ...
        #     'oc_model_state_dict': gnn_order_dispatch,
        ...
        print(f"Epoch [{epoch+1}/{CONFIG['epochs']}], Loss: {loss.item():.4f}, Reward: {loss:.4f}, Preference: {omega}")
